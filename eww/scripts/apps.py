...
def mathApp(content, copy):
    ...
    entry = {
        ...
        "icon": icon_path,
        "comment": content,
        "desktop": f"--version; echo '{copy}' | xclip -selection clipboard & notify-send -i 'accessories-calculator' -a 'Calculator' 'Copied to Clipboard' '{content}'",
    }
    return entry
...
def math(query):
    try:
        ...
        return [True, mathApp(f"{temp} = {result}", result)]
...
def update_cache(all_apps, preferred_apps):
    data = {"math": [False], "apps": all_apps, "preferred": preferred_apps}
...
def get_cached_entries():
    ...
    return {"math": [False], "apps": all_apps, "preferred": preferred_apps}

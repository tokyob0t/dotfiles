...
def get_desktop_entries(file_path):
    ...
    icon_path = getIcon(parser.get("Desktop Entry", "Icon", fallback=None))
...
def highlight(text, query):
    ...
    startTag = '<span foreground="#8f9091" font-weight="900">'
    endTag = '</span>'

    for keyword in query.split():
        text = text.replace(keyword, f"{startTag}{keyword}{endTag}")
    return text
